...
void main() {
    ...
    // vec3 purpleColor = vec3(0.0, 0.0, 0.0);
    // vec3 greeColor = vec3(1.0, 1.0, 0.0);
    // vec3 uvColor = vec3(1.0, vUv);
    // float strength = step(0.9, sin(cnoise(vUv * 10.0) * 20.0));
    // vec3 mixColor = mix(greeColor, purpleColor, strength);
    ...
    // vec3 uvColor = vec3(1.0, vUv);
    // float strength = step(0.9, sin(cnoise(vUv * 10.0) * 20.0));
    // vec3 mixColor = mix(greeColor, uvColor, strength);
    // gl_FragColor = vec4(mixColor, 1.0);
